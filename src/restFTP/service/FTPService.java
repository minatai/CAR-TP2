...
public class FTPService {
	 ...
	 *
	 * @param directory
	 *            affiche la liste de ce dossier
	 * @return
	 */
	public List<String> listFiles(final String directory) {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("Erreur: Impossible de lister le dossier"
					+ directory);
		}
	...
	public List<String> listDirectory(final String directory) {
		...
		try {
			final FTPFile[] directoriesname = this.ftpClient
					.listDirectories(directory);

			for (int i = 0; i < directoriesname.length; i++) {
				System.out.println(directoriesname[i].getName());
				directories.add(directoriesname[i].getName());
			}
		} catch (final IOException e) {
			System.out.println("Erreur: Impossible d'afficher les dossier");
			e.printStackTrace();
		}
		return directories;
	}
